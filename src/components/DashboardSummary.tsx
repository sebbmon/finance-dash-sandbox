...
export function DashboardSummary() {
  const { totalIncome, totalExpense, currentBalance, isHydrated, preferences } = useFinance();
  ...
  return (
    <div className="grid gap-4 grid-cols-1 md:grid-cols-3">
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{formatCurrency(currentBalance, preferences.currency)}</div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
